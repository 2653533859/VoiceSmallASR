/// 真实第三方 OpenAI-compatible API 英/日视频翻译验收。
///
/// 这是显式手工验收，不属于默认测试集；必须通过
/// `--dart-define-from-file` 传入仓库外的本地配置文件，测试不会把 API Key
/// 写入日志或报告。运行前需要把模型和 `test_wavs/en.mp4`、`test_wavs/ja.mp4`
/// 放进应用私有模型目录。
///
/// 配置文件示例（不要提交）：
///
/// ```json
/// {
///   "VSASR_TRANSLATION_API_KEY": "provider-key",
///   "VSASR_TRANSLATION_API_ENDPOINT": "https://provider.example/v1/chat/completions",
///   "VSASR_TRANSLATION_API_MODEL": "provider-model",
///   "VSASR_TRANSLATION_TARGET_LANGUAGE": "zh-CN"
/// }
/// ```
///
/// ```bash
...
void main() {
  ...
  testWidgets('真实第三方 API 英日视频生成双语 SRT 并显示视频叠加', (WidgetTester tester) async {
    const String apiKey = String.fromEnvironment('VSASR_TRANSLATION_API_KEY');
    if (apiKey.trim().isEmpty) {
      fail(
        '缺少 VSASR_TRANSLATION_API_KEY；请用 --dart-define-from-file 传入仓库外的配置文件',
      );
    }
    const String endpoint = String.fromEnvironment(
      'VSASR_TRANSLATION_API_ENDPOINT',
      defaultValue: kDefaultTranslationApiEndpoint,
    );
    const String model = String.fromEnvironment(
      'VSASR_TRANSLATION_API_MODEL',
      defaultValue: kDefaultTranslationApiModel,
    );
    const String targetLanguage = String.fromEnvironment(
      'VSASR_TRANSLATION_TARGET_LANGUAGE',
      defaultValue: kDefaultTranslationTargetLanguage,
    );
    const String glossary = String.fromEnvironment(
      'VSASR_TRANSLATION_GLOSSARY',
    );
      ...
      for (final _TranslationCase item in _cases) {
        ...
        try {
          ...
          final File srtFile = File(
            p.join(output.path, '${item.language}-translated.srt'),
          );
          await srtFile.writeAsString(srt);
          // ignore: avoid_print
          print('${item.language} → $targetLanguage 双语 SRT：${srtFile.path}');
          ...
          await _waitUntil(
            () =>
                (player.position.inMicroseconds - position.inMicroseconds)
                    .abs() <
