...
| 界面（`lib/src/ui/`） | ✅ M1、M2、M3、M5、M11、M12 完成，M13 字幕批量时间偏移、搜索替换、阅读速度检查、翻译术语表、服务商预设、播放器字幕样式、视频配套字幕导出、文件转写性能诊断、批量/实时性能汇总和持续性能历史已完成：文件转写、实时字幕、视频播放、字幕联动、外部字幕加载、批量转写/翻译/导出、批量队列恢复、字幕校对编辑、项目保存/打开/最近项目 |
