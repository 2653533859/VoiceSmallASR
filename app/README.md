...
| 翻译层（`lib/src/translation/`） | ✅ 第三方 OpenAI-compatible provider、批量/重试/进度、批量文件翻译复用 provider、双语导出、目标语言持久化、术语表、API 连接测试、文件/实时/视频字幕翻译已完成；真实网络验收按个人使用范围主动跳过 |
| 界面（`lib/src/ui/`） | ✅ M1、M2、M3、M5、M11、M12 完成，M13 字幕批量时间偏移、搜索替换、阅读速度检查和翻译术语表已完成：文件转写、实时字幕、视频播放、字幕联动、外部字幕加载、批量转写/翻译/导出、批量队列恢复、字幕校对编辑、项目保存/打开/最近项目 |
