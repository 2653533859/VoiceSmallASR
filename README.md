...
每句都定稿且时间戳连续不重叠。
后续计划包括真实英/日视频翻译验收、Android 真机性能、Windows 完整模型 e2e 与用户桌面运行验证，以及签名发布。
...
Windows Release 与 Inno Setup 安装包已由 `.github/workflows/windows-build.yml` 在 Windows runner 上构建通过，CI 同时检查运行时 DLL 和模型文件排除，并通过无模型桌面 smoke 验证 AAC 解码与 MP4 播放；完整模型 e2e 和用户桌面运行仍待验收。

音频解码上两端有意不同：Python 端调系统 ffmpeg，Flutter 端 wav 走纯 Dart 直读、压缩格式与视频交给平台原生解码（macOS 用 AVFoundation，Android 用 MediaCodec，Windows 用 Media Foundation）——`ffmpeg_kit_flutter` 已弃养且从不支持 Windows。macOS 那份已编译并端到端跑通；Android 的 Kotlin 已在 API 35 ARM64 模拟器端到端验证但尚未真机运行，Windows 的 C++ 已在 MSVC CI 编译并通过无模型桌面 smoke，但完整模型 e2e 和用户桌面运行仍未验证。
