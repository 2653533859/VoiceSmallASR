...
- M6 首期设置页已完成：语言、线程数、ITN、临时结果间隔、VAD 断句参数和 DeepL API Key 可保存；普通设置用 `shared_preferences`，API Key 用 `flutter_secure_storage`，启动时恢复配置。
- 当前下一步是实现模型下载/删除/占用空间与离线模式设置，用真实网络完成英/日视频验收，同时补 Android/Windows 的构建与运行验证。

## 已验证结果

- Python：`pytest` 105 项通过，`ruff check .` 通过。
- Flutter：`flutter analyze` 通过，132 项单测通过。
...
- M6 测试覆盖：3 项 API Key 安全存储测试、3 项普通设置持久化测试、2 项设置页测试，以及 1 项配置变更后 worker 重启测试。
