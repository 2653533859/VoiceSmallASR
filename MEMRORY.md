...
- M10 验收入口：新增 `app/integration_test/device_acceptance_test.dart`，模型缺失时实际下载并校验，使用模型自带 `yue.wav` 记录文件 RTF，按需测视频播放和真实麦克风实时积压，并通过 `dart:io ProcessInfo` 记录各阶段进程 RSS 当前值/峰值；新增 [`M10_DEVICE_ACCEPTANCE.md`](M10_DEVICE_ACCEPTANCE.md) 区分自动化报告与安装包手工清单。`dart analyze`、`flutter analyze --no-pub`、模型管理单测 2/2 和 Flutter 全量测试 234 项通过，提交 `344034e` 的 Windows 回归 run `32050804305` 通过，但尚无真实 Android/Windows 用户桌面数据。
