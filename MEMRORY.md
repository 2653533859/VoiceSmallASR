...
- M6 的 API Key 安全存储基础已完成：接入 `flutter_secure_storage`，固定 DeepL 存储键，支持去空白、非空校验、读取和删除；macOS Debug/Release Keychain entitlements 已补齐。
- 当前下一步是把安全存储接入设置页、用真实网络完成英/日视频验收，同时补 Android/Windows 的构建与运行验证。

## 已验证结果

- Python：`pytest` 105 项通过，`ruff check .` 通过。
- Flutter：`flutter analyze` 通过，126 项单测通过。
...
- M6 测试覆盖：3 项 API Key 安全存储测试，覆盖去空白、空值拒绝、固定存储键与删除。
