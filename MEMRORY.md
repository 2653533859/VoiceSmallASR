...
- M19 第一阶段已完成：将播放器叠加层、播放控制、字幕列表、播放列表视图和字幕缓存管理对话框从 `video_page.dart` 拆到 `app/lib/src/video/`，页面仅改为组装这些组件并转发原有事件；快捷键、字幕显示模式、播放倍速、播放列表操作和缓存管理相关测试通过，`flutter analyze` 通过。下一步提取视频转写/播放列表业务协调器，再更新 M19 验收状态。
