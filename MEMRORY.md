...
- M4 可选真实翻译验收入口已补齐：`app/integration_test/api_translation_acceptance_test.dart` 使用仓库外 JSON 配置调用 OpenAI-compatible API，覆盖英/日视频转写、批量翻译、双语 SRT 和视频叠加字幕；默认测试不联网、不读取密钥，旧 DeepL provider/回归测试仍保留兼容性。
