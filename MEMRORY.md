...
- M15 发布一致性正在推进：应用版本已升为 `1.0.4+4`；Release workflow 现会校验版本格式、tag commit 与 `GITHUB_SHA`、已有 Release 的 `BUILD_INFO.txt` Commit，并从 Android APK、macOS `Info.plist` 和 Windows 可执行文件反读版本，阻止不一致资产覆盖 Release。当前 `v1.0.3` 的历史 tag/资产错配已记录在 [`NEXT_DEVELOPMENT_PLAN.md`](NEXT_DEVELOPMENT_PLAN.md)，下一步使用新建的 `v1.0.4` tag 发布修正版。
