...
- 当前后续计划见 [`NEXT_DEVELOPMENT_PLAN.md`](NEXT_DEVELOPMENT_PLAN.md)：M8 发布质量基线代码已完成，M9 无签名翻译体验已完成，M10 已完成环境审计但真机/用户桌面验收仍待条件具备，M11 项目保存、字幕导入、首页项目管理、Android SAF、自动保存、异常恢复和媒体重新定位已完成，M12 多文件队列与批量翻译首项已完成，下一步是按原文件名安全导出、文本缓存和队列恢复。

## 已验证结果

- Python：`pytest` 107 项通过，`ruff check .` 通过。
- Flutter：`flutter analyze` 通过，189 项单测通过；Python 端 107 项测试通过。
...
- M12 队列与批量翻译首项：新增批量处理页和顺序多文件转写，支持去重、每文件进度、暂停/继续、取消和失败重试；批量翻译复用同一个 provider 和现有分批/重试逻辑，逐文件显示翻译状态，单项失败不阻塞后续文件，取消通过操作代次阻止迟到译文写回，Android SAF 无本地路径时明确拒绝而不静默丢条目；全量 Flutter 测试增至 189 项。按原文件名导出、文本缓存和队列恢复仍待完成。
