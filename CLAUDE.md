...
| Flutter 三端客户端（Windows/macOS/Android） | `app/` | **M1、M2、M3 已完成，M4 翻译基础、批量流程与双语导出已完成**：文件转写、实时字幕、视频播放与字幕联动（117 项 `flutter test` + 7 项 `integration_test`）。macOS 真实 mp4 已验收，Android/Windows 与在线翻译服务商仍待验证/决策 |
...
flutter test                   # 117 项，不需要模型也不需要设备
