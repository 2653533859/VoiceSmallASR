...
| Flutter 三端客户端（Windows/macOS/Android） | `app/` | **M1、M2、M3、M5 已完成，M4 翻译基础、批量流程、双语导出、DeepL provider 与应用内翻译工作流已完成，M6 设置页与模型管理首期已完成**：文件转写、实时字幕、视频播放、字幕联动与字幕校对编辑（149 项 `flutter test` + 7 项 `integration_test`）。macOS 真实 mp4 已验收，Android/Windows 与真实翻译验收仍待验证 |
