...
| Flutter 三端客户端（Windows/macOS/Android） | `app/` | **M1、M2、M3、M5 已完成，M4 翻译基础、批量流程、双语导出、第三方 API provider 与应用内翻译工作流已完成，M6 设置页与模型管理首期已完成，M7 已完成 macOS 无签名包、Android APK/AAB 和 Windows Release/安装包构建验证，M8 发布质量基线代码已落地，M9 无签名翻译体验已完成，M11 项目文件数据层、首页保存/打开、最近项目、Android SAF、自动保存与异常恢复首项已完成**：文件转写、实时字幕、视频播放、字幕联动与字幕校对编辑（175 项 `flutter test` + Android API 35 模拟器端到端 7 项 + Windows runner 完整模型端到端 7 项）。macOS/Android 模拟器/Windows CI 真实 mp4 已验收，Android 真机性能和 Windows 用户桌面运行仍待验证；真实翻译验收为个人使用范围外的可选项 |
...
flutter test                   # 175 项，不需要模型也不需要设备
