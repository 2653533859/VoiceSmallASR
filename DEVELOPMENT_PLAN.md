...
- [x] 音频解码层（方案见 §5 已决策 1）：Dart 侧 `AudioDecoder` 接口 + wav 纯 Dart 直读；
  ...
  - [x] Windows 原生实现（Media Foundation IMFSourceReader，
        **已在 GitHub Actions windows-2022 runner 编译，并通过桌面 smoke 验证 AAC 解码；完整模型 e2e 和用户桌面运行仍未验证**）
...
- [ ] **Windows 完整模型 e2e —— 待执行**：`.github/workflows/windows-build.yml` 已提供手动 `run_full_e2e=true` 开关；模型准备脚本使用三源 fallback、每源有限重试、低速超时、大小校验和 Actions cache，运行时通过 `VSASR_MODEL_DIR` 使用外部模型目录，避免复制进应用沙盒
