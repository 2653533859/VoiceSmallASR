...
- [ ] **Windows 完整模型 e2e —— 待执行**：`.github/workflows/windows-build.yml` 已提供手动 `run_full_e2e=true` 开关；模型准备脚本使用三源 fallback、低速超时、大小校验和 Actions cache，运行时通过 `VSASR_MODEL_DIR` 使用外部模型目录，避免复制进应用沙盒
