...
- [x] Android release APK 发布 CI 安装启动 smoke 已接入：`release.yml` 在 Android 产物校验后用 API 35 `google_apis` x86_64 模拟器执行同一安装/冷启动脚本，job 总时限 30 分钟；首次云端结果待下一次 Tag 发布或手动发布工作流记录，不替代 Android 真机验收
