...
│   │   │   ├── speaker_diarization.dart   说话人分离后台推理与字幕映射
│   │   │   └── speaker_diarization_model_manager.dart  分离模型下载/校验/缓存
│   │   ├── audio/
...
- [x] 代码审查修复：验收入口按字幕段中点跳转，避免把 `start + end` 误当作播放时间；使用本地 mock OpenAI-compatible endpoint 已完成英/日视频转写、双语 SRT、视频跳转和叠加字幕回归，真实第三方网络验收仍按个人使用范围单独执行
...
- [x] Windows 修复提交回归：推送 `bd64894` 后的 run [`32088850142`](https://github.com/2653533859/VoiceSmallASR/actions/runs/32088850142) 通过 Flutter analyze、Release 构建、桌面 smoke、`ass/libass` 硬字幕 smoke、产物校验、安装包首次启动和 artifact 上传；完整模型 E2E 按 workflow 仅在手动 `run_full_e2e=true` 时执行，本次明确跳过
...
- [x] API 35 ARM64 模拟器复测更新（非真机）：当前工作树用 JDK 17 构建并重新运行统一入口，模型首次准备 84,782 ms，目录 241,150,289 bytes，完整性通过，2 线程文件 RTF `0.026418`，解码/识别耗时 10/136 ms；模型准备后 RSS 当前 442,556,416 bytes，文件 worker 加载后峰值 758,063,104 bytes。视频和麦克风未传参数而明确跳过，详细数据见 [`M10_DEVICE_ACCEPTANCE.md`](M10_DEVICE_ACCEPTANCE.md)，该结果不替代 Android 真机、实时 RTF 或厂商 Codec 验收
...
- [x] 批量翻译复用 provider；逐文件显示翻译状态/进度，失败不阻塞后续文件，取消不会写回迟到译文
- [x] 按原文件名安全导出 SRT/VTT/JSON/TXT；同批次重名自动追加序号，保存器对既有目标交给系统显示覆盖确认，取消后停止后续导出
- [x] 已完成文本翻译缓存：按媒体路径、源识别结果、目标语言、provider 配置和术语表生成指纹，命中前确认复用，损坏或不匹配按未命中处理
- [x] 批量任务持久化和异常退出后的队列恢复：队列变化防抖保存，退出前强制写入；恢复前由用户确认，进行中状态降级为可继续状态，放弃时清理快照
...
- [x] 翻译术语表：设置页保存每行 `原词=译词`，校验重复/非法条目，并注入文件、实时、视频和批量翻译请求；术语表变化会隔离批量翻译缓存
- [x] 服务商预设：设置页可保存、选择和删除 endpoint、模型、目标语言和术语表组合；预设不保存 API Key，损坏条目会被跳过
- [x] 字幕样式与视频配套字幕导出：视频页可持久化字号、颜色、位置，并按视频文件名导出 SRT/VTT/JSON/TXT
- [x] 文件转写性能诊断：记录模型准备、解码、识别和总耗时、RTF、采样点、模型占用、平台与 ASR/VAD 配置，并支持查看/导出 JSON
- [x] 批量与实时识别性能汇总：批量页和实时页支持查看/导出 JSON，分别统计队列汇总与实时会话采样、时长、耗时和 RTF
...
设置页还支持可选翻译术语表和服务商预设：术语表按每行 `原词=译词` 校验并持久化，预设保存 endpoint、模型、目标语言和术语表组合；两者都不保存 API Key，批量缓存指纹会包含术语表内容。
