...
> 更新日期：2026-08-18　　基线版本：v1.0.1　　下一目标：v1.0.2
...
- [x] 硬字幕视频编码：桌面调用本机 FFmpeg/ASS，Android 调用系统 MediaCodec + OpenGL + MediaMuxer，均生成带原文、译文、说话人标签并复用字幕样式的 MP4；Android 支持 AAC 音轨直通和 SAF `content://` 输出（API 26+），新增 MethodChannel 单测、页面回归测试和跨平台真实验收脚本。Android 真实设备、非 AAC 音轨和桌面带 libass FFmpeg 仍待验收。
...
当前进度：M13 自动说话人分离和桌面/Android 硬字幕编码代码已完成代码审查、相关测试和验收脚本；`flutter analyze`、Flutter 相关测试、Android Kotlin 编译和 Android API 35 ARM64 模拟器硬字幕真实编码均已通过。本机 macOS 真模型/桌面硬字幕真实编码验收分别受无签名集成测试开发证书和 FFmpeg 缺少 libass 限制；Android 真机与 Windows 用户桌面仍待具备环境后验收。下一步是完成真实设备验收，并根据设备结果补充非 AAC 音轨或编码兼容性。
