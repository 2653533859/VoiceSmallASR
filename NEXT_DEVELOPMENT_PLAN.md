...
完成 1 小时/约 400 MiB 合成视频的连续音轨分块解码、播放器打开/推进、两个播放列表条目切换、播放器语义 widget 回归和设备资源记录；该结果不代表真实语音转写质量，
也不能替代 macOS 睡眠唤醒、辅助功能语义树、Android 真机和 Windows 用户桌面验收，因此本阶段任务仍保持未完成。随后补充视频播放列表协调器的应用生命周期暂停/恢复处理：应用隐藏或睡眠时停止继续读取新音频，保存当前检查点，唤醒后重新排队；短于 30 秒的进度也会在暂停收尾时写入检查点，并由协调器回归测试覆盖。
...
在 M18 完成前，暂不建议继续扩展更多视频字幕功能或升级核心音视频依赖；个人使用范围内也不投入商店签名、公证和正式发布证书。
