...
- Flutter 端：文件转写、实时字幕、视频播放、字幕编辑、双语导出、第三方 API 翻译、设置页和模型管理已完成；项目文件数据层、首页保存/打开、最近项目、Android SAF 字节读取、自动保存、异常恢复、媒体缺失重新定位、SRT/VTT/JSON 字幕导入和 M12 多文件队列/批量翻译/安全导出/文本缓存/队列恢复已接入；M13 已加入字幕批量时间偏移、搜索替换、阅读速度检查、翻译术语表、服务商预设、播放器字幕样式、视频配套字幕导出、桌面硬字幕视频编码、文件转写性能诊断、批量/实时性能汇总报告、持续性能历史和自动说话人分离；`flutter analyze` 与 `flutter test` 233 项通过。
...
- 相关解析、首页导入和视频外部字幕加载测试已覆盖，Flutter 全量测试为 233 项。
...
- [x] 硬字幕视频编码（桌面首期）：视频页可选择输出路径，调用本机 FFmpeg 的 ASS 滤镜生成带原文、译文和说话人标签的 MP4，并复用字幕样式；新增编码器单测、页面回归测试和显式真实 FFmpeg 验收脚本。Android 暂不内置编码，后续仍需引入跨平台编码方案。
...
当前进度：M13 自动说话人分离和硬字幕视频编码桌面首期已完成代码审查、相关测试和验收脚本；本机 macOS 真模型/硬字幕真实编码验收分别受无签名集成测试开发证书和 FFmpeg 缺少 libass 限制，Android 真机与 Windows 用户桌面仍待具备环境后验收。下一步是补齐 Android 内置硬字幕编码方案并完成各平台真实验收。
