...
- Flutter 端：文件转写、实时字幕、视频播放、字幕编辑、双语导出、第三方 API 翻译、设置页和模型管理已完成；项目文件数据层、首页保存/打开、最近项目、Android SAF 字节读取、自动保存、异常恢复和媒体缺失重新定位已接入；`flutter analyze` 与 `flutter test` 176 项通过。
...
- 已接入首页“保存项目 / 打开项目”和“最近项目”；最近项目持久化最多 8 个路径，Android SAF 文件按字节读取并在应用支持目录保留稳定缓存；Flutter 全量测试增至 176 项。
