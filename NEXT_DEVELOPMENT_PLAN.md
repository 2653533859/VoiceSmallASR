...
- [ ] 将实时字幕、当前视频转写、用户主动文件转写和播放列表预转写统一交给显式任务调度器，并按“麦克风实时字幕 → 当前播放视频 → 用户主动任务 → 后台预缓存”分配优先级。
- [ ] 后台预缓存遇到当前播放或麦克风任务时自动暂停，显示排队、处理中、已取消等状态。
- [ ] 按已完成音频块保存转写检查点，应用异常退出后可以从最近检查点继续，不重复处理整段视频。
- [ ] 增加无进展检测和可重试错误；解码或识别在设定时间内没有推进时，释放当前会话并允许重新开始。
- [ ] 记录队列等待时间、解码/识别速度、峰值内存、已处理时长和最后进度，用于定位“卡住不动”。
- [ ] 使用 400 MB 以上、1～2 小时视频，以及至少 3 个播放列表条目进行长时间回归。

验收标准：长视频不会无限停在某一秒；取消后 2～3 秒内释放会话；内存不随视频时长持续增长；应用重启后可以继续未完成转写。
...
在 M16 和 M17 完成前，暂不建议继续扩展更多视频字幕功能或升级核心音视频依赖；个人使用范围内也不投入商店签名、公证和正式发布证书。
