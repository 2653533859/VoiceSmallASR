...
> 更新日期：2026-09-13　　发布基线：v1.0.4　　当前源码版本：1.0.5+5

## 2026-09-12 功能扩展执行计划

功能扩展按“先统一任务状态，再提高字幕生产效率，最后增加高级媒体能力”的顺序推进。现有识别、播放器、字幕编辑和批量处理架构继续保留；新增功能优先复用当前控制器与缓存，不引入新的全局状态框架。

### 第一轮：统一后台任务中心与稳定性（P0/P1，执行中）

目标：识别、翻译和批量任务运行时，用户可以从主窗口统一查看状态并执行安全操作，页面切换不影响后台工作。

- [x] 主窗口右上角增加任务中心入口和活动任务数量徽标。
- [x] 汇总当前文件识别、普通批量队列、实时字幕、识别 worker 占用和等待队列；显示文件名、阶段、进度与失败条目数量。
- [x] 提供当前文件任务取消、批量任务暂停/取消/进入详情、实时字幕停止操作；失败批量任务可进入原页面重试。
- [x] 覆盖空闲状态、后台文件识别、取消、批量队列汇总和进入详情回归。
- [x] 将视频页逐句翻译、硬字幕编码和文件夹自动处理接入同一任务快照；文件夹处理控制器已提升到首页生命周期，返回主界面后任务继续运行，可从任务中心查看或暂停。
- [x] 增加最近失败任务与脱敏诊断入口，最多保留 10 条；只显示任务类型、文件名、时间和清理后的摘要，不记录字幕文本、完整路径、网络地址或 API Key。
- [ ] 使用真实视频验证处理期间打开任务中心、设置、播放器和字幕页面不会卡住或中断任务。

2026-09-13 部分验收：三个 5–10 秒 H.264/AAC MP4 已通过 macOS 原生音轨解码、播放推进、播放列表切换和无语音视频流式转写；10 秒视频打开耗时 130 ms，完整转写管线耗时 2,876 ms，队列等待 0 ms，峰值 RSS 为 768,376,832 bytes。另用模型测试语音循环生成仓库外的 10 分钟 H.264/AAC 临时视频：600 秒音轨分成 59 块，首条定稿 3,345 ms，总耗时 33,400 ms，输出 117 段并正常收尾，转写阶段峰值 RSS 为 811,155,456 bytes。自动化回归确认文件夹处理页退出后任务继续完成。原 `Downloads/vv` 素材已不存在，macOS 屏幕捕捉接口又阻止了本轮自动界面操作；循环语音也不能替代自然长视频质量验证，因此任务中心、设置和翻译人工验收继续保留未完成。

验收标准：所有长任务至少能显示名称、阶段和安全的停止入口；同一任务不会重复计数；取消后的迟到结果不写回；窄窗口和桌面窗口均无布局溢出。

### 第二轮：字幕校时、统一翻译记忆与编辑效率（P1）

目标：减少反复翻译和逐条校时的人工成本，优先强化已有字幕编辑工作流。

- [x] 在现有整体偏移、质量检查和选区重新识别基础上增加按选定锚点整体对齐；保持字幕间隔、时长、译文和 token 时间戳，并支持撤销。
- [x] 实现保守的 VAD 分段校时建议器：仅在字幕与语音段一一对应、边界调整未超阈值且结果无重叠时返回建议；10 分钟循环语音的 117 段和 60 秒自然语音的 14 段分别注入 0.75 秒错位后，平均边界误差均降至 0。
- [ ] 使用自然语音和真实错位字幕验证漏检、多检及分段数量不一致场景，再决定是否把 VAD 自动对齐加入正式界面。
- [ ] 将现有批量翻译缓存扩展为文件、视频、实时定稿和批量任务共用的翻译记忆；作用域包含服务商、endpoint、模型、目标语言和术语表。
- [ ] 字幕编辑器增加原文/译文双栏校对、仅显示未翻译或质量异常条目，以及可配置的上一条/下一条快捷键。
- [ ] 增加 ASS 导出，并评估 YouTube 字幕和 Final Cut Pro XML；每种格式先用最小样本验证时间轴、换行、双语和说话人字段。
- [ ] 为自动校时、缓存失效、双栏编辑和新增导出格式补充可撤销性与往返解析测试。

验收标准：相同配置与原文不重复调用翻译服务；配置变化不误用旧译文；自动校时不会制造倒序或重叠；编辑操作可撤销，导出后时间轴与当前项目一致。

### 第三轮：播放器工具、OCR、离线翻译与媒体资料库（P2/P3）

目标：补充高价值的媒体处理能力；每项先做独立原型和资源测量，再决定是否进入正式界面。

- [ ] 播放器增加逐帧前后移动、A-B 循环、标记和截图；按视频记住播放位置、旋转角度与字幕显示设置。
- [ ] 硬字幕 OCR 先实现手动选择画面区域和抽帧识别原型，再处理时间合并、重复文本和编辑器导入；不得与语音字幕自动混合覆盖。
- [ ] 离线翻译先比较模型体积、首句延迟、内存和中日英质量；只有目标设备资源可接受时才提供在线/离线切换。
- [ ] 媒体资料库保存路径、缩略图、字幕状态、语言和处理时间，支持按文件夹和关键词过滤；媒体文件仍只保存引用，不复制大视频。
- [ ] 画中画放在播放器基础工具稳定后评估；云端同步、在线模型市场和主题扩展继续暂缓。

验收标准：播放器工具不破坏现有方向键、全屏和字幕叠加；OCR 与离线模型可独立卸载且不增加基础安装包体积；资料库中媒体缺失时可重新定位，不阻塞字幕编辑。
...
本轮已新增任务中心组件、页面任务注册表和最近失败任务；视频逐句翻译支持从任务中心取消，硬字幕编码显示进度，文件夹任务可离开处理页继续运行。组合回归确认识别期间可依次打开设置、切换播放器和使用任务中心，任务不会因导航中断；字幕编辑器已开始第二轮并完成锚点整体校时与保守 VAD 对齐建议器。全量 Flutter 测试基线更新为 407 项。OCR、离线模型和资料库仍在第一轮真实视频人工验证后再启动。

## 2026-09-10 下一阶段执行计划

当前工作树包含 27 个已跟踪文件和 8 个新文件，共计约 2352 行新增、334 行删除；其中已经组合了文件夹顺序处理、视频字幕策略、播放器交互、字幕质量检查和翻译提速。下一阶段先建立可回退的稳定基线，再验证真实长任务，最后优化翻译调度并准备下一版本，不继续在未收口的工作树上叠加独立大功能。

### 第一轮：当前工作树收口（P0）

目标：把现有功能变成经过审查、测试、安装验证和版本控制保护的开发基线。

- [ ] 按文件夹批处理、视频播放、字幕编辑、翻译与设置五条调用链审查当前 diff，重点检查取消后的迟到写回、页面销毁、任务并发、缓存覆盖和播放器状态切换。
- [ ] 修复审查发现的问题后，运行 `flutter analyze --no-pub`、397 项 Flutter 全量测试和 `git diff --check`；不以单个页面测试代替全量回归。
- [ ] 在已安装的 macOS 包中完成一轮人工 smoke：文件夹自然排序与已有字幕跳过、默认关闭字幕/翻译、连续左右键跳转、四个旋转角度、翻译首句显示、翻译或识别期间打开设置。
- [ ] 同步 README、开发计划和验收记录；确认文档中的测试数、构建号、DMG 校验值与实际产物一致。
- [ ] 经用户明确要求后提交并推送，确认远端提交和本地 HEAD 一致；本轮不创建发布 Tag。

验收标准：代码审查没有未解决的 P0/P1 问题；自动化检查全部通过；macOS 安装包可启动并完成上述 smoke；工作树中的本轮改动已由 Git 提交保护。

### 第二轮：真实视频长任务与后台可操作性（P0/P1）

目标：针对实际使用中“播放或处理时卡死、界面不能操作、任务中断后丢进度”的风险，取得可复现数据并修复根因。

- [ ] 选取至少一个真实含语音的长视频和三个普通视频，覆盖播放、转写、逐句翻译、播放列表切换、反复跳转、旋转、全屏和字幕开关；素材不足 1 小时时明确记录实际时长，不把短视频结果写成长视频结论。
- [ ] 分别记录首句字幕时间、首句译文时间、总处理时间、峰值 RSS、解码会话数、队列等待时间和失败阶段；诊断报告不得保存用户字幕原文或 API Key。
- [ ] 验证识别与翻译运行期间设置、播放、字幕查看、只读报告和任务取消仍可使用；发现主 isolate 上的文件 I/O、JSON 处理或批量状态更新时，仅迁移已证实造成阻塞的路径。
- [ ] 验证暂停、取消、应用退出、系统睡眠唤醒和网络错误后的检查点恢复；迟到结果不得覆盖新任务，单个条目失败不得阻塞播放列表后续条目。
- [ ] 根据实测调整转写分块、状态刷新频率、预缓存让行和缓存上限；调整必须保留前后指标，避免只增加并发导致内存和限流恶化。

验收标准：连续处理期间界面仍能切页和响应播放控制；取消可在有界时间内完成；恢复后不重复或覆盖已完成字幕；内存不随已处理时长持续线性增长；所有修复都有对应回归测试或验收记录。

### 第三轮：翻译调度与 v1.0.5 交付（P1/P2）

目标：在当前“首批 1 句、后续每批 8 句、最多 3 路并发”的基础上，根据服务实际表现降低首句等待和限流失败，并形成下一版可安装产物。

- [ ] 将固定后续并发升级为有上限的自适应策略：正常响应逐步提高吞吐，429、5xx、超时或高延迟时主动降低并发；认证、模型和地址错误立即停止，不进行无效重试。
- [ ] 保持逐批写回和播放位置优先；已经有译文或命中同配置缓存的句子不重复发送，取消后返回的响应不写入当前字幕。
- [ ] 在设置或任务状态中显示首批耗时、已完成句数、当前并发、重试等待和最近错误，所有 endpoint 与错误信息继续脱敏。
- [ ] 使用同一组不含敏感文本的固定字幕样本比较优化前后首批延迟、总耗时、吞吐和请求失败数；只有实测改善且无回归时保留参数调整。
- [ ] 完成代码审查、全量测试、macOS 安装 smoke，并准备 `v1.0.5` 的变更记录、三端构建和 SHA-256 清单；Android 真机或 Windows 用户桌面不可用时，在发布说明中保留未验收状态。

验收标准：首句译文继续逐句出现；限流时吞吐可自动回落且原文显示不受阻；相同配置不会重复翻译；macOS 包可复现构建并安装；发布资产、源码提交、版本号和校验清单一致。

### 暂缓功能

画中画、可自定义快捷键、云端项目同步、在线模型市场和界面主题扩展暂不进入这三轮。播放器旋转后续只补充真实全屏/长视频发现的缺陷，不单独扩展为视频编辑功能。等三轮验收完成，再依据实际使用频率决定下一批功能。
...
第二轮：真实视频稳定性与后台可操作性
  ↓
第三轮：自适应翻译调度与 v1.0.5 交付
```

每轮继续按“分析 → 定位 → 最小修改 → 相关测试 → 全量回归 → 文档同步”执行。提交、推送和发布只在用户明确要求时进行。

## 2026-09-08 工作流与界面改进计划

当前源码已经完成文件夹顺序转写与翻译、视频文件夹播放、配套字幕优先加载、纵向播放列表，以及播放页默认关闭字幕和翻译。播放画面支持按 90° 顺时针旋转，字幕、控制层和时间轴保持正常方向。进入批处理前会暂停正在播放的视频，播放进度也不再触发首页整页刷新；文件夹任务支持立即暂停当前识别。字幕生成或翻译期间仍可打开设置页，保存、导出、性能报告和字幕质量检查等只读操作保持可用；会影响当前任务的参数继续保持只读。普通字幕翻译首批请求 1 句，后续每批 8 句并保持最多 3 路并发；视频页从播放位置附近开始，已有译文自动跳过，每批返回后立即更新字幕。Flutter 自动化测试基线为 397 项。上述最新改动仍在工作树中，后续修改必须保留这些内容，并在进入下一轮前完成统一验证。

接下来的目标是把现有能力组合成清晰、可恢复、可检查的工作流。按照依赖关系分三轮推进：

### 第一轮：视频字幕策略与界面收敛

- [x] 将字幕处理拆为“关闭 / 仅加载已有字幕 / 缺失时识别”三种模式，默认关闭。
- [x] 翻译保持独立开关；字幕关闭时翻译不可开启，切换关闭时终止后台字幕任务。
- [x] 收敛视频页顶部工具栏，保留打开、添加、字幕策略和翻译等高频操作，其余进入菜单。
- [x] 播放列表显示字幕来源、处理状态和错误状态；宽窗口使用右侧列表，窄窗口使用弹窗。
- [x] 覆盖同名字幕、缓存命中、无字幕、识别失败、模式切换和窄窗口布局测试。

验收标准：打开视频或文件夹后不发生字幕读取、识别和翻译；选择“仅加载已有字幕”不会启动识别；选择“缺失时识别”才允许对无字幕视频转写。

### 第二轮：统一批量任务中心

- [x] 将多文件批量和文件夹批量的状态、失败重试和持久化语义统一。
- [x] 文件夹队列支持异常退出恢复、暂停/继续、逐项重试和状态过滤。
- [x] 增加输出目录、字幕格式、双语/原文和重名处理策略。
- [x] 明确展示已有字幕跳过原因，并在处理前与写入前防止覆盖。

验收标准：单项失败不阻塞后续任务；应用重启后可恢复未完成队列；已有字幕不会被覆盖；取消后不写入迟到结果或半成品。

### 第三轮：字幕质量与格式能力

- [x] 汇总时间重叠、异常阅读速度、长空白、空文本、重复文本和疑似语言错误。
- [x] 质量问题可直接定位时间轴并进入选区重新识别。
- [x] 播放页统一加载 SRT/VTT/ASS/SSA 与语言后缀字幕，并支持选择视频内嵌字幕轨。
- [x] 文件夹任务支持整批语言策略和单文件覆盖，检测语言冲突时提示用户。

验收标准：质量结果可定位、可修复、可撤销；外部字幕选择规则在批量页和播放页一致；真实媒体验收不记录或提交用户字幕内容。

第一轮验收（2026-09-08）：`flutter analyze --no-pub` 无问题，`flutter test --no-pub` 379 项全部通过。

第二轮验收（2026-09-08）：文件夹队列使用应用私有目录中的版本化 JSON 快照，处理中断后恢复为等待状态；页面支持完成当前文件后暂停、单项重新排队及等待/失败/完成/跳过筛选。输出可选择媒体旁或独立目录、SRT/VTT/JSON/TXT、原文或翻译双语；独立目录重名时可跳过或自动编号。媒体旁已有字幕仍在扫描、处理前和写入前检查，写入采用独占创建并在失败时清理半成品。`flutter analyze --no-pub` 无问题，`flutter test --no-pub` 384 项全部通过。

第三轮验收（2026-09-09）：Studio 将阅读速度检查扩展为统一字幕质量报告，问题可定位播放位置或带入选区重新识别。字幕导入和视频配套字幕支持 SRT/VTT/ASS/SSA，配套文件按同名优先、格式优先级和语言后缀确定；播放后端默认关闭视频内嵌字幕，并在字幕工具中提供轨道选择。文件夹任务可设置整批识别语言和单文件覆盖，选定语言与段语言检测不一致时在完成状态中提示复核。识别中切换字幕策略不会把增量结果当作完整字幕，显示原文、译文或双语也不会改变加载策略。播放中进入批处理会先暂停播放器，避免隐藏视频解码与 ONNX 转写同时占用资源；首页不再随播放进度整页重建，文件夹任务可以立即取消当前识别并保留等待队列。字幕生成或翻译期间可以打开设置页查看状态，保存、导出、性能报告和字幕质量检查可与后台任务并行，识别参数仍在任务结束前锁定。普通字幕翻译采用首批 1 句、后续每批 8 句、最多 3 路并发的自适应策略，视频页优先翻译播放位置附近并跳过已有译文。当前服务实测 17 句耗时 6.32 秒、吞吐 2.69 句/秒，首批译文 3.79 秒返回。`flutter analyze --no-pub` 无问题，`flutter test --no-pub` 397 项全部通过；另用包含 H.264、AAC 和日文 SubRip 内嵌轨的合成 MKV 在 macOS 原生播放器后端完成轨道枚举、选择和关闭集成验收。

播放旋转补充验收（2026-09-10）：播放页「更多」菜单支持按 90° 顺时针循环旋转画面；视频使用旋转后的宽高重新布局，字幕、播放控制和时间轴通过反向旋转保持正常方向。视频页回归测试覆盖初始角度、90° 旋转和控制层方向，`flutter analyze --no-pub` 无问题，`flutter test --no-pub` 397 项全部通过。

## 当前工作树（2026-09-05）

三轮改进已新增工作台校对工具、worker/实时录音资源保护、普通文件与 WAV 分块处理、日常 CI 和离线质量评测入口。实现范围、指标含义、真实模型验证及剩余验收统一见 [三轮改进记录](THREE_ROUND_IMPROVEMENTS.md)。以下历史基线及测试数按各记录日期理解，不作为新发布结论。

本计划建立在三端个人使用版本已经可以打包发布的基础上。后续优先保证安装包可验证、无开发者证书环境可用，再增加项目管理和批量处理能力。

## 当前基线

- Python 端：离线多语种识别、CLI、实时 VAD、时间戳和字幕导出已完成；`pytest` 117 项通过。
- Flutter 端：文件转写、实时字幕、视频播放、字幕编辑、双语导出、第三方 API 翻译、设置页和模型管理已完成；项目文件数据层、首页保存/打开、最近项目、Android SAF 字节读取、自动保存、异常恢复、媒体缺失重新定位、SRT/VTT/JSON 字幕导入和 M12 多文件队列/批量翻译/安全导出/文本缓存/队列恢复已接入；M13 已加入字幕批量时间偏移、搜索替换、阅读速度检查、翻译术语表、服务商预设、播放器字幕样式、视频配套字幕导出、桌面与 Android 硬字幕视频编码、文件转写性能诊断、批量/实时性能汇总报告、持续性能历史和自动说话人分离，M14 已加入视频流式字幕、播放列表和后续字幕预缓存；M16 已加入共享转写任务调度、优先级让行、长视频背压、断点检查点、无进展看门狗、资源收尾和诊断报告；M17 已接入 Android/Windows 连续原生解码协议；M18 已加入播放列表持久化、拖拽排序、单项取消/重试/移除、字幕缓存清单/清理、检查点恢复展示、配置/媒体变更标记、访问时间淘汰和容量上限；M19 已完成视频侧和首页侧业务协调器拆分；M21 与 M23 的可自动化代码任务已完成；当前 `flutter analyze` 与 `flutter test` 通过，Flutter 测试共 336 项。
...
- 当前工作树正在补强并发处理和 Android 硬字幕长任务：识别器改为可复用 worker 池，调度器按容量允许多个任务并行，批量转写按容量分发任务；Android 硬字幕编码会启动前台服务，编码渲染复用字幕位图。还新增了可配置的 ONNX provider（`auto`/`cpu`/`nnapi`/`coreml`）。这组改动已通过 `flutter analyze` 和 273 项 Flutter 测试，但当前主机未安装 Java Runtime，尚未完成 Android Kotlin 编译；NNAPI/CoreML、前台服务和实际并发内存占用也尚未在真实设备验收。因此它不构成 M20 完成结论。
...
### M21 · 播放器交互与字幕体验收敛（P1，代码完成）

目标：把字幕和播放操作集中到播放器上下文中，降低顶部工具区拥挤和字幕控件重叠的概率。

任务：

- [x] 将现有字幕显示能力收敛到播放器控制条，统一为字幕关闭、仅原文、仅译文和双语四种显示模式，并让当前模式有清晰的选中状态。
- [x] 将播放倍速、左右方向键快退/快进 10 秒、字幕开关和字幕翻译开关统一到同一套播放状态；输入焦点位于文本框、编辑器或弹窗时不拦截方向键。
- [x] 对窄窗口、长文件名和字幕较长的场景做响应式布局，控制条不得遮挡进度条、字幕或全屏按钮。
- [x] 为显示模式、倍速、键盘快捷键和窗口尺寸增加 Widget/集成回归，覆盖播放器销毁后迟到事件不再写回页面。

验收标准：常用字幕操作无需离开播放器；原文/译文切换即时生效；左右键每次准确移动 10 秒且不超出视频边界；窄窗口下控件不重叠，现有播放列表、转写和缓存任务不受影响。

实现结果：字幕模式改为带选中状态的播放器菜单，自动翻译开关移入控制条；窄窗口使用可换行的控制组，文本编辑器和弹窗焦点不会触发播放器方向键。相关 Widget 回归已通过。

### M22 · 长视频与播放列表可靠性（P1，待真实验收）
...
- [x] 已有代码与自动化回归覆盖暂停/恢复、应用重启、取消、源视频变化、检查点续写和单个播放列表条目失败；恢复时只从有效检查点继续，不重复覆盖已完成字幕。磁盘空间不足和真实系统睡眠唤醒仍需设备验证。
- [x] 将无进展、解码器异常、模型初始化失败和缓存写入失败转换为用户可操作的错误状态，并保证当前条目失败不会让整个播放列表崩溃。
...
### M23 · 第三方翻译 API 的可用性与可诊断性（P2，代码完成）

目标：在保持用户自定义 API 和手动模型名能力的前提下，减少模型列表、限流和网络波动对字幕工作的影响。

任务：

- [x] 保留“获取模型列表”和手动输入模型名两条路径；模型列表失败时显示脱敏后的原因，不清空用户当前模型配置。
- [x] 为文件、实时字幕、视频字幕和批量翻译统一超时、有限重试、取消和限流退避策略，并区分可重试错误与配置错误。
- [x] 明确翻译任务的缓存作用域（服务商、endpoint、模型、目标语言和术语表），配置变化后不能误用旧译文。
- [x] 补充 mock provider 的连接测试、模型列表、部分失败、取消、重试和响应数量校验；真实第三方 API 仍不作为自动化门禁，也不提交 API Key。

验收标准：模型列表可选时能直接使用，列表不可用时手动模型仍可用；实时和视频字幕翻译失败不阻塞原文显示；重试不会重复写入或污染其他视频/批次缓存。

实现结果：`TranslationRequestPolicy` 统一限制单次请求时长，并只对 408/409/425/429、5xx 和瞬时错误退避重试；认证、模型、地址和响应格式错误不会重试。任务取消后返回的译文不会写回字幕。

### M24 · 字幕缓存、队列与发布维护（P2/P3，部分待验收）
...
- [x] 完善缓存清单中的占用、最后访问时间、检查点状态和清理结果；清理、重新定位和配置变更不得误删当前任务或有效字幕。
- [x] 已将 Flutter 全量回归以及既有长视频/播放列表/翻译相关回归纳入发布工作流；工作流会校验 tag、构建提交、包内版本、模型排除、SHA-256 清单和三端安装启动。尚未因此创建新发布。
- [x] 只在确有兼容性证据时升级音视频依赖；继续维持无开发者证书 macOS 包、个人 Android 包和 Windows 未签名安装包的交付范围。

验收标准：后台预缓存可暂停、恢复和取消，缓存占用受上限约束；发布资产与源码提交一致，三端包可按现有脚本重建并通过对应 smoke。

实现结果：缓存管理器会显示检查点状态、占用、最后访问时间和最近一次清理统计，容量淘汰按最后访问时间执行；真实长视频数据、实际三端构建和发布资产仍需在 M20 环境具备后执行，不能由单元测试替代。

### M20 之后的执行闸门

2026-09-04 按当前优先级已提前完成 M21、M23 的代码任务，以及 M22、M24 可自动化的可靠性和缓存回归；这不跳过 M20 的真实设备结论。后续先补齐设备报告，再完成 M22/M24 中依赖真实长语音视频、磁盘空间与三端发布产物的验收项。

```text
完成 M21 / M23 代码与 M22 / M24 自动化回归
  ↓
完成 M20 真实设备报告
  ↓
完成 M22 长语音视频与 M24 三端发布验收
```
