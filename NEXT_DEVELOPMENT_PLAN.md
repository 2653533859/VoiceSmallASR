...
- Flutter 端：文件转写、实时字幕、视频播放、字幕编辑、双语导出、第三方 API 翻译、设置页和模型管理已完成；项目文件数据层、首页保存/打开入口和最近项目已接入；`flutter analyze` 与 `flutter test` 171 项通过。
...
- 已接入首页“保存项目 / 打开项目”和“最近项目”；最近项目持久化最多 8 个可由 `dart:io` 读取的本地路径，并在保存/打开成功后置顶；Flutter 全量测试增至 171 项。
- Android SAF 的 `content://`/provider 路径暂不加入最近项目，待跨平台 URI 文件访问适配；其余待完成项为自动保存、异常恢复、原媒体缺失提示和 SRT/VTT/JSON 字幕导入。
...
- 多文件队列、暂停、取消、失败重试。
- 每个文件独立显示解码、识别和翻译状态。
- 批量翻译复用现有分批 provider，避免重复创建 HTTP client。
- 支持按原文件名导出 SRT/VTT/JSON/TXT，并避免覆盖已有文件。
- 对已完成文本建立本地缓存，重复翻译前先确认是否复用。
