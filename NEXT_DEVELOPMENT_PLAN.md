...
- Flutter 端：文件转写、实时字幕、视频播放、字幕编辑、双语导出、第三方 API 翻译、设置页和模型管理已完成；项目文件数据层与首页保存/打开入口已接入；`flutter analyze` 与 `flutter test` 168 项通过。
...
- 已接入首页“保存项目 / 打开项目”，保存结果包含媒体引用、识别配置、字幕时间轴和译文；Flutter 全量测试增至 168 项。
- 待完成“最近项目”、自动保存、异常恢复、原媒体缺失提示和 SRT/VTT/JSON 字幕导入。
